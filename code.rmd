...
while (converged==FALSE) {
  ...
  result <- optim(beta, loglik_complete)
  beta_new <- result$par
...
$$\begin{aligned}F_X(x;\lambda)&=1-x^{-\lambda}\\F_Y(y;\mu)&=1-y^{-\mu}\end{aligned}$$
with $x,y \ge 1$ and $\lambda,\mu>0$.
...
while (converged == FALSE) {
  # E-step
  E_Z <- (theta[1] * fX(Y, theta[2])) / (theta[1] * fX(Y, theta[2]) + (1 - theta[1]) * fY(Y, theta[3]))
  
  # M-step
  theta_new <- theta
  theta_new[1] <- mean(E_Z)
  lambda_result <- suppressWarnings(optim(par = theta[2], fn = function(lambda) -sum(E_Z * (log(lambda) - (lambda + 1) * log(Y)))))
  theta_new[2] <- lambda_result$par
  mu_result <- suppressWarnings(optim(par = theta[3], fn = function(mu) -sum((1 - E_Z) * (log(mu) - (mu + 1) * log(Y)))))
  theta_new[3] <- mu_result$par
...
Y_sorted <- sort(Y)
density_values <- theta[1] * fX(Y_sorted, theta[2]) + (1 - theta[1]) * fY(Y_sorted, theta[3])

# Plot histogram and density
hist(Y, freq = FALSE, breaks = hist_breaks(Y), col = "grey", border = "black", xlab = "Y", ylab = "Density", main = "Histogram with Estimated Density Superimposed")
lines(Y_sorted, density_values, col = "blue", lwd = 2)
```
